...
function TrustScoreBadge({ score, tone }: { score: number; tone: CardTone }) {
  ...
  return (
    <span
      ...
      className={`inline-flex h-8 w-[108px] shrink-0 items-center justify-center rounded-xl border px-2 py-0.5 text-[12px] font-semibold tabular-nums transition-transform duration-200 ${toneCls.scoreBadge}`}
    >
      {score}
...
function FeedLeading(props: {
  ...
  return (
    <div className="flex min-w-0 flex-1 gap-3">
      <span
        className={`mt-0.5 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border text-[18px] leading-none ${toneCls.iconWrap} ${toneCls.icon}`}
        aria-hidden
      >
        {m.glyph}
      </span>
      <div className="min-w-0 flex-1 space-y-1">
        ...
        <p id={headlineId} className="text-balance text-[17px] font-semibold leading-tight tracking-tight text-slate-900 md:text-[18px]">
          {m.headline}
        </p>
        <p className="text-[13px] font-medium leading-snug text-slate-600">{m.technicalLabel}</p>
        <p className="break-all pt-0.5 text-[14px] font-medium text-slate-900 md:text-[15px]" title={domainFullTitle}>
          {domainLine || "—"}
        </p>
      </div>
    </div>
...
export function CompactOverviewFeedLinkCard(props: CompactOverviewFeedBaseProps & { bgClassName?: string }) {
  ...
  return (
    <Link href={href} className={`${shell} group ${bgClassName ?? ""}`} aria-labelledby={headlineId} aria-label={ariaLabel}>
      <span
        aria-hidden
        className={`pointer-events-none absolute -left-8 top-1/2 h-28 w-28 -translate-y-1/2 rounded-full bg-gradient-radial blur-2xl ${toneCls.surfaceGlow}`}
      />
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between md:gap-6">
        ...
        <div className="hidden min-w-[10.25rem] shrink-0 flex-col items-end gap-2 text-right md:flex">
...
export function CompactOverviewFeedArticleCard(props: CompactOverviewFeedBaseProps & { trailingActions?: ReactNode }) {
  ...
  return (
    <article
      className={`${m.articleClass} ${toneCls.accentBar} fraudly-motion group relative min-h-[188px] rounded-2xl border border-slate-200/60 bg-white/95 p-4 shadow-[0_8px_30px_rgba(15,23,42,0.06)] backdrop-blur-[1px] before:absolute before:inset-y-3 before:left-0 before:w-1 before:rounded-r md:p-5 hover:-translate-y-[1px] hover:border-slate-300/80 hover:shadow-[0_12px_34px_rgba(15,23,42,0.10)]`}
    >
      <span
        aria-hidden
        className={`pointer-events-none absolute -left-8 top-1/2 h-28 w-28 -translate-y-1/2 rounded-full bg-gradient-radial blur-2xl ${toneCls.surfaceGlow}`}
