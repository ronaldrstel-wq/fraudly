...
export function TrustSummaryMetrics({ normalized, variant = "check" }: TrustSummaryMetricsProps) {
  ...
  const trustLabel =
    normalized.trustScore == null ? flow.siteOutcome.suppressedTrustMeter : `${normalized.trustScore} / 100`;
  ...
  return (
    ...
    <dl className="mt-8 grid gap-3 sm:grid-cols-3">
      <div className={trustCardClass}>
        <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">{flow.checkPage.metricsTrustScore}</dt>
      ...
      <div className={secondaryCardClass}>
        <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">{flow.checkPage.metricsDomainAge}</dt>
        <dd className="mt-1 text-lg font-bold leading-snug text-slate-900">{normalized.domainAge.display}</dd>
      </div>
      <div className={secondaryCardClass}>
        <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">{flow.checkPage.metricsSsl}</dt>
        <dd className="mt-1 text-base font-semibold text-slate-900">{normalized.ssl.display}</dd>
      </div>
    </dl>
    {confidenceBadges.length > 0 ? (
      <div className="mt-3">
        <TrustDataConfidenceBadgeRow badges={confidenceBadges} ariaLabel={flow.scanResult.dataConfidenceAria} />
      </div>
    ) : null}
    </>
  );
